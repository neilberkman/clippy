...
// DarwinClipboardManager implements ClipboardManager for macOS using Objective-C APIs
type DarwinClipboardManager struct{}

// newClipboardManager creates a new clipboard manager for macOS
func newClipboardManager() ClipboardManager {
	return &DarwinClipboardManager{}
}

// CopyFile copies a single file reference to clipboard
func (m *DarwinClipboardManager) CopyFile(path string) error {
...
func (m *DarwinClipboardManager) CopyFiles(paths []string) error {
...
func (m *DarwinClipboardManager) CopyText(text string) error {
...
func (m *DarwinClipboardManager) GetFiles() []string {
	var count C.int
...
func (m *DarwinClipboardManager) GetText() (string, bool) {
...
func (m *DarwinClipboardManager) GetUTIForFile(path string) (string, bool) {
	cPath := C.CString(path)
	defer C.free(unsafe.Pointer(cPath))
...
func (m *DarwinClipboardManager) GetClipboardTypes() []string {
	var count C.int
...
func (m *DarwinClipboardManager) GetClipboardDataForType(typeStr string) ([]byte, bool) {
...
func (m *DarwinClipboardManager) ContainsType(typeStr string) bool {
	cType := C.CString(typeStr)
	defer C.free(unsafe.Pointer(cType))
...
func (m *DarwinClipboardManager) UTIConformsTo(uti, parentType string) bool {
	cUTI := C.CString(uti)
...
func (m *DarwinClipboardManager) GetClipboardContent() (*ClipboardContent, error) {
	...
	if files := m.GetFiles(); len(files) > 0 {
		...
		filePath := files[0]
		uti, _ := m.GetUTIForFile(filePath)
	...
	types := m.GetClipboardTypes()
	for _, typeStr := range types {
		// Look for specific image types first
		if isImageUTI(typeStr) {
			if data, ok := m.GetClipboardDataForType(typeStr); ok {
				return &ClipboardContent{
					Type:   typeStr,
					Data:   data,
					IsText: false,
